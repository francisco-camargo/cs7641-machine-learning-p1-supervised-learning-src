...
Seems like even with a random seed defined the compute time values are still
random. This may be reason enough to average over many trials.
"""
...
def plot_learning_curve(estimator,
    ...
    # axes[0].set_title(project_alias + ' data, ' + model_choice + ' Learning Curve')
    # if ylim is not None:
    ...
    print('Test score: ' + str(round(100*train_mean,1)) + '+/-' + str(round(100*train_std,1)))
    
    test_mean  = test_scores_mean[-1]
    test_std   = test_scores_std[-1]
    print('Test score: ' + str(round(100*test_mean,1)) + '+/-' + str(round(100*test_std,1)))
    
    fig, ax = plt.subplots()
    ax.figure.set_size_inches(fig_dim_x, fig_dim_y)
    plt.title(project_alias + ' data, ' + model_choice + ' Learning Samples', fontdict = {'fontsize' : fontsize_ticks})
    ...
    if savefig:
        filename= project_alias + '_' + model_choice + '_' + 'learning_samples'
        plt.savefig(filepath+filename+'.png', format='png')
    
    
    # Fit Times
    mask = fit_times_mean.argsort()
    x = fit_times_mean[mask]
    y = test_scores_mean[mask]
    half_band = test_scores_std[mask]
    low = y - half_band
    hig = y + half_band
    
    fig, ax = plt.subplots()
    ax.figure.set_size_inches(fig_dim_x, fig_dim_y)
    plt.title(project_alias + ' data, ' + model_choice + ' Learning Fit Time', fontdict = {'fontsize' : fontsize_ticks})
    plt.xlabel('Fit Time', fontsize=fontsize)
    plt.ylabel(scoring + ' score', fontsize=fontsize)
    plt.fill_between(x, low, hig, alpha=0.1)
    plt.plot(x, y, label='Training Fitting Time', lw=lw)
    ax.tick_params(direction='in')
    plt.xticks(fontsize=fontsize_ticks)
    plt.yticks(fontsize=fontsize_ticks)
    fig.tight_layout(pad=tight_pad)
    ...
    if savefig:
        filename= project_alias + '_' + model_choice + '_' + 'learning_fit_time'
        plt.savefig(filepath+filename+'.png', format='png')
...
if __name__ == "__main__":
    ...
    train_sizes = np.logspace(-2, 0, 10)
    ylim = (0.7, 1.01)
    xscale = 'log'
